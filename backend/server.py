...
def join():
    ...
    mute_audio = data.get('audio')  # 1 or 0
    mute_video = data.get('video')  # 1 or 0
    room_id = 1
...
def on_join_room(data):
    ...
    # Notify existing users of the new user and send them existing stream offers
    if room_id not in users_in_room:
        ...
        for existing_sid in users_in_room[room_id]:
            if existing_sid != sid:  # Don't send to self
                emit("user-connect", {"sid": existing_sid, "name": names_sid[existing_sid]}, room=sid)
